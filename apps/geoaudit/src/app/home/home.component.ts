...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  notifications$: Observable<
    Array<Notification>
  > = this.notificationEntityService.entities$.pipe(
    map((notification) => {
      ...
      for (var i = 0; i < jobSurveys.length; i++) {
        ...
        switch (a.status.name) {
          case 'COMPLETED':
            ...
            var marker_i = L.marker(
              new L.LatLng(a.geometry['lat'], a.geometry['lng']),
              { icon: busIcon, title: jobSurveys[i].name }
            );
            var select_popup =
              '<h2> survey ' + jobSurveys[i].name + '</h2><hr>';
            select_popup +=
  ...
  private initMap(): void {
    ...
    this.abrioxEntityService.getAll().subscribe(
      (marker_data) => {
        ...
        for (var i = 0; i < this.abrioxes.length; i++) {
          ...
          // if no geometry data, skip it
          if (!a.testpost || !a.testpost.geometry || !this.abrioxes[i].name)
            continue;
          // if against Layer rule, skip it
          ...
          switch (a.condition.name) {
            case 'WORKING':
              ...
              var marker_i = L.marker(
                new L.LatLng(
                  a.testpost.geometry['lat'],
                  a.testpost.geometry['lng']
                ),
                { icon: busIcon, title: this.abrioxes[i].name }
              );
              var select_popup =
                '<h2> abriox ' + this.abrioxes[i].name + '</h2><hr>';
              select_popup +=
                '<button data-btn="detail" data-type="abrioxes" data-id="' +
                this.abrioxes[i].id +
                '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
              select_popup +=
                '<button data-btn="notes" data-type="abrioxes" data-id="' +
                this.abrioxes[i].id +
                '" class="sp_button">Notes<span class="detail_button_icon">></span></button>';
              select_popup +=
                '<button data-btn="drive" data-type="abrioxes" data-id="' +
                this.abrioxes[i].id +
                '" class="sp_button">DriveTo<span class="detail_button_icon">></span></button>';
    ...
    this.testpostEntityService.getAll().subscribe(
      (marker_data) => {
        marker_data.sort(function (a, b) {
          if (a.tp_actions['date'] < b.tp_actions['date']) return -1;
          return 1;
        });
        ...
        for (var i = 0; i < this.testposts.length; i++) {
          ...
          // if no condition data, skip it
          if (!a.actions || !a.actions.condition || !a.actions.condition.name)
            continue;
          ...
          switch (a.actions.condition.name) {
            case 'WORKING':
              if (flag1 == 0) {
                ...
                var marker_i = L.marker(
                  new L.LatLng(a.geometry['lat'], a.geometry['lng']),
                  { icon: busIcon, title: this.testposts[i].name }
                );
                var select_popup =
                  '<h2> testpost ' + this.testposts[i].name + '</h2><hr>';
                select_popup +=
                  '<button data-btn="detail" data-type="testposts" data-id="' +
                  this.testposts[i].id +
                  '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
                select_popup +=
                  '<button data-btn="history" data-type="testposts" data-id="' +
                  this.testposts[i].id +
                  '" class="sp_button">Historical data<span class="detail_button_icon">></span></button>';
                select_popup +=
                  '<button data-btn="notes" data-type="testposts" data-id="' +
                  this.testposts[i].id +
                  '" class="sp_button">Notes<span class="detail_button_icon">></span></button>';
                select_popup +=
                  '<button data-btn="drive" data-type="testposts" data-id="' +
                  this.testposts[i].id +
                  '" class="sp_button">DriveTo<span class="detail_button_icon">></span></button>';
    ...
    this.trEntityService.getAll().subscribe(
      (marker_data) => {
        marker_data.sort(function (a, b) {
          if (a.date_installation < b.date_installation) return -1;
          return 1;
        });
          ...
          switch (a.actions.condition.name) {
            case 'WORKING':
              if (flag1 == 0) {
                ...
                var marker_i = L.marker(
                  new L.LatLng(a.geometry['lat'], a.geometry['lng']),
                  { icon: busIcon, title: this.trs[i].name }
                );
                var select_popup = '<h2> tr ' + this.trs[i].name + '</h2><hr>';
                select_popup +=
                  '<button data-btn="detail" data-type="trs" data-id="' +
                  this.trs[i].id +
                  '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
                select_popup +=
                  '<button data-btn="history" data-type="trs" data-id="' +
                  this.trs[i].id +
                  '" class="sp_button">Historical data<span class="detail_button_icon">></span></button>';
                select_popup +=
                  '<button data-btn="notes" data-type="trs" data-id="' +
                  this.trs[i].id +
                  '" class="sp_button">Notes<span class="detail_button_icon">></span></button>';
                select_popup +=
                  '<button data-btn="drive" data-type="trs" data-id="' +
                  this.trs[i].id +
                  '" class="sp_button">DriveTo<span class="detail_button_icon">></span></button>';
    ...
    this.resistivityEntityService.getAll().subscribe(
      (marker_data) => {
        ...
        for (var i = 0; i < this.resistivities.length; i++) {
          ...
          if (!a || !a.geometry || !a.geometry['lat'] || !a.geometry['lng'])
            continue;
          // if against Layer rule, skip it
          if (!a.footer || !a.footer.approved) continue;
          ...
          var marker_i = L.marker(
            new L.LatLng(a.geometry['lat'], a.geometry['lng']),
            { icon: busIcon, title: 'this.resistivities[i].name' }
          );
          var select_popup =
            '<h2> resistivity ' + this.resistivities[i].reference + '</h2><hr>';
          select_popup +=
            '<button data-btn="detail" data-type="resistivities" data-id="' +
            this.resistivities[i].id +
            '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
          select_popup +=
            '<button data-btn="notes" data-type="resistivities" data-id="' +
            this.resistivities[i].id +
            '" class="sp_button">Notes<span class="detail_button_icon">></span></button>';
          select_popup +=
            '<button data-btn="drive" data-type="resistivities" data-id="' +
            this.resistivities[i].id +
            '" class="sp_button">DriveTo<span class="detail_button_icon">></span></button>';
        ...
        for (var i = 0; i < this.surveys.length; i++) {
          ...
          switch (a.status.name) {
            case 'COMPLETED':
              ...
              var marker_i = L.marker(
                new L.LatLng(a.geometry['lat'], a.geometry['lng']),
                { icon: busIcon, title: this.surveys[i].name }
              );
              var select_popup =
                '<h2> survey ' + this.surveys[i].name + '</h2><hr>';
              select_popup +=
                '<button data-btn="detail" data-type="surveys" data-id="' +
                this.surveys[i].id +
                '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
    ...
    this.map.on(L.Draw.Event.CREATED, function (e) {
      ...
      if (type === 'marker') {
        ...
        var select_popup =
          '<select class="popup_select"><option value="testpost">Testpost</option><option value="tr">Tr</option><option value="survey">Survey</option><option value="resistivity">Resistivity</option></select>';
        select_popup +=
          '<button class="popup_detail_btn">Details<span class="detail_button_icon">></span></button>';
        // new_marker.bindPopup(select_popup , select_popup_css);
        var popup = L.popup({
          className: 'add_marker_popup',
          closeButton: false,
        }).setContent(select_popup);
        ...
        $('.popup_detail_btn').on('click', (e) => {
          ...
          switch (now_op_val) {
            case 'abriox':
              instance.abrioxEntityService
                ...
                .subscribe((notification) => {
                  ...
                  if (insert_id > 0) {
                    ...
                    var marker_i = L.marker(
                      new L.LatLng(popup_geometry.lat, popup_geometry.lng),
                      { icon: busIcon, title: '' }
                    );
                    var select_popup =
                      '<h2> ' + now_op_val + ' ' + '' + '</h2><hr>';
                    select_popup +=
                      '<button data-btn="detail" data-type="' +
                      now_op_val +
                      '" data-id="' +
                      insert_id +
                      '" class="sp_button">Details<span class="detail_button_icon">></span></button></a>';
                    if (now_op_val == 'testpost' || now_op_val == 'tr')
                      select_popup +=
                        '<button data-btn="history" data-type="' +
                        now_op_val +
                        '" data-id="' +
                        insert_id +
                        '" class="sp_button">Historical data<span class="detail_button_icon">></span></button>';
                    select_popup +=
                      '<button data-btn="notes" data-type="' +
                      now_op_val +
                      '" data-id="' +
                      insert_id +
                      '" class="sp_button">Notes<span class="detail_button_icon">></span></button>';
                    select_popup +=
                      '<button data-btn="drive" data-type="' +
                      now_op_val +
                      '" data-id="' +
                      insert_id +
                      '" class="sp_button">DriveTo<span class="detail_button_icon">></span></button>';
              ...
              break;
            case 'testpost':
              instance.testpostEntityService
                .add({
                  geometry: {
                    lat: popup_geometry.lat,
                    lng: popup_geometry.lng,
            ...
            case 'tr':
              ...
              break;
            case 'resistivity':
              instance.resistivityEntityService
                .add({
                ...
                .subscribe((notification) => {
                  ...
                  if (insert_id > 0) {
                    ...
                    instance.router.navigate([
                      '/home/resistivities/' + insert_id,
                    ]);
                  }
                });
              break;
            case 'survey':
              instance.surveyEntityService
                .add({
                  geometry: {
                    lat: popup_geometry.lat,
                    lng: popup_geometry.lng,
    ...
    this.map.on('click', (e) => {
      ...
      if (this.url) {
        ...
        this.store.dispatch(
          MapActions.addClickMarker({ clickMarker: e.latlng })
        );
